    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Created MongoDB monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error creating MongoDB Monitor.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Updated MongoDB monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error updating MongoDB Monitor.');
      }
    });
  };
