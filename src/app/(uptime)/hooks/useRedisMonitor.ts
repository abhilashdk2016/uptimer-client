    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Created Redis monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error creating Redis Monitor.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Updated Redis monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error updating Redis Monitor.');
      }
    });
  };
