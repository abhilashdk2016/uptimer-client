    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            router.push('/status');
            successToast('Created HTTP monitor successfully.');
          }
        }
      } catch (error) {
        errorToast('Error creating HTTP Monitor.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            router.push('/status');
            successToast('Updated HTTP monitor successfully.');
          }
        }
      } catch (error) {
        errorToast('Error updating HTTP Monitor.');
      }
    });
  };
