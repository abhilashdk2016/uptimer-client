    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Created TCP monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error creating TCP Monitor.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            successToast('Updated TCP monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        errorToast('Error updating TCP Monitor.');
      }
    });
  };
