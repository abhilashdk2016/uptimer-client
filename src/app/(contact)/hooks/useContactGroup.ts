    ...
    startTransition(async () => {
      try {
        if (emails.length > 0 && notificationGroup.groupName) {
          ...
          if (result) {
            successToast('Created notification group successfully.');
            router.push('/contact');
          }
        }
      } catch (error) {
        errorToast('Error creating notification group.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        if (emails.length > 0 && notificationGroup.groupName) {
          ...
          if (result) {
            router.push('/contact');
            successToast('Updated notification group successfully.');
          }
        }
      } catch (error) {
        errorToast('Error updating notification group.');
      }
    });
  };
