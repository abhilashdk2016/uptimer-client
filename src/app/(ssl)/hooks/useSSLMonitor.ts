    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            router.push('/ssl');
            successToast('Created SSL monitor successfully.');
          }
        }
      } catch (error) {
        errorToast('Error creating SSL Monitor.');
      }
    });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            router.push('/ssl');
            successToast('Updated SSL monitor successfully.');
          }
        }
      } catch (error) {
        errorToast('Error updating SSL Monitor.');
      }
    });
  };
